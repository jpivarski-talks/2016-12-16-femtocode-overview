...
\item[CPU execution:] 70 (13) seconds to apply a simple operation \textcolor{gray}{(``for'' loop with {\tt -O0} ({\tt -O3}), AWS ``t2.medium'' 2.4 GHz)},
...
\vspace{0.1 cm}
\textcolor{gray}{(Probably close to ``theoretical optimum,'' before distributed processing. Multi-user cache would be very important!)}
\end{uncoverenv}
\end{frame}
...
\vspace{0.5 cm}
We'd have accept the security risk of physicists running arbitrary code downloaded from a server, because type-checking is unrestricted Python code. Also, if it depends on libraries (like ROOT), the physicist user would have to have those versions of the libraries installed.
